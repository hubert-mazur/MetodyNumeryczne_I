...
std::pair<double, double> *metoda_trapezow(double xn, double vn, double dT, double alpha)
{
    ...
    while (true)
    {
        ...
        a21 = (-dT / 2.0) * (-2 * alpha * xn_1 * vn_1 - 1);
        a22 = 1.0 - (dT / 2.0) * alpha * (1.0 - pow(xn_1, 2));
...
{
    double k1x = vn;
    double k1v = alpha * (1 - pow(xn, 2)) * vn - xn;
...
void timeControl(double x0, double v0, double dT0, double S, int p, double Tmax, double alpha, std::pair<double, double> *funct(double, double, double, double), std::string metoda)
{
    ...
    FILE *(fileArray)[] = {fopen(&(metoda + std::to_string(TOL[0]) + ".dat")[0], "w"), fopen(&((metoda + std::to_string(TOL[1]) + ".dat"))[0], "w")};
    ...
    for (int j = 0; j < 2; j++)
    {
        ...
        for (double i = 0.0; i <= Tmax;)
        {
            ...
            step_n_2_1 = funct(xn, vn, 2 * dT, alpha);
            Ex = (step_n_2_2->first - step_n_2_1->first) / (pow(2, p) - 1);
            Ev = (step_n_2_2->second - step_n_2_1->second) / (pow(2, p) - 1);
            ...
            dT = pow((S * TOL[j]) / (fmax(fabs(Ex), fabs(Ev))), 1.0 / (p + 1)) * dT;
            delete step_n1_2;
            delete step_n_2_1;
            delete step_n_2_2;
        }
